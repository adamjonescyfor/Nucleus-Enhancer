...
| `NucleusTemplateChangeRequest__c` | "Suggest an edit" — analysts propose template changes; admins review with a diff and Apply/Reject ([spec](salesforce-change-request-object.md)) | ⬜ optional |
...
> ⚠️ **Keep `Content__c` (and the version content field) as a *Long* Text Area, not *Rich* Text Area.** The editor supports embedded (base64) images, and Salesforce's rich-text sanitiser strips inline `data:` images on save — so a Rich Text Area would silently drop template images. Formatting still renders fine in the manager regardless of the field type.
...
- **Values:** the **team codes**, exactly as in `NucleusTeam__c.TeamCode__c` — one picklist entry per active team. (Codes, **not** display names.) ⚠️ **Keep this in sync:** the manager's team picker offers every active team, so whenever you **add a new team**, add its code here too — otherwise assigning a template to that team will fail.
...
- The one exception: **Create + Read** on `NucleusTemplateUsage__c` (insert-only audit log; no Edit/Delete) — **and field-level security set to editable** on its `Template_Name__c`, `Record_Id__c`, `Record_URL__c` fields. ⚠️ Object Create *without* the FLS is the usual failure: the describe reports `createable: false`, and every usage write is rejected with `CANNOT_INSERT_UPDATE_ACTIVATE_ENTITY`, so the org-wide log records nothing for ordinary users (only admins). The extension writes a usage record for **every** connected user, so this perm must reach all analysts, not just admins.
...
7. ☐ (Optional) Suggest-an-edit — create `NucleusTemplateChangeRequest__c` + permissions ([spec](salesforce-change-request-object.md)) so analysts can propose template edits for admins to review.
8. ☐ Readable record names (section above) — nice-to-have.
9. ☐ Optional cleanup: delete the unused custom **Changed By / Changed By Email** fields (the extension stopped writing them; standard Created/Last Modified By replaced them).
10. ☐ After everyone is on the current extension version: tell the developer, who can raise the Cloudflare worker's `MIN_CLIENT_VERSION` to retire old builds.

**Data entry (anyone with access, once step 2 is done):**
11. ☐ A `NucleusTeamMember__c` record per user (right team; the user's name as the record Name once step 8 is done). A user who belongs to **several teams** gets one record per team.
12. ☐ Tick `IsAdmin__c` on the chosen admins' Team Member records (remember: admins need BOTH this flag — which unlocks the manager UI — and the permission set from step 3, which is what Salesforce enforces).
